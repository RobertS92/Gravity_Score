...
# Conservative market-value display knots. A score over 90 requires truly
# exceptional league-relative APY/salary; the broad middle maps to 50–75.
...
def _commercial_market_score(
    ...
    """Blend model estimate with an authoritative observed APY/salary anchor."""
    ...
    if observed_usd > 0:
        ...
        # A validated contract/salary is more authoritative than a noisy model,
        # while retaining a small model contribution for continuity.
...
def _trusted_social_reach(raw: dict[str, Any]) -> float:
    ...
    if _social_audience_is_trusted(raw):
        return ig + tt + tw
    # Untrusted snapshots often attach team/org Twitter; keep Instagram only.
    return ig
...
def score_athlete(req: ScoreAthleteRequest) -> ScoreAthleteResponse:
    ...
    impact_key = model_key("athlete", sport, "impact")
    fallback_used = True
    model_version = req.model_version or "1.0.0-heuristic"
    gravity = None
    quality = None
    value_score = None
    value_score_source = None
    dollar_p50 = None
    ...
    # Value Score = winning impact. Prefer impact ML champion when present.
    impact_bundle = loader.resolve(impact_key)
    if impact_bundle:
        pred = _predict_from_bundle(impact_bundle, raw)
        if pred:
            value_score = min(100.0, max(5.0, float(pred["primary"])))
            value_score_source = f"{impact_key}/{impact_bundle.version}"
    ...
    if (
        ...
        # Only BPXVR / rank-only paths use cohort display calibration.
        # Commercial ML Gravity is already dollar-ordered — do not re-rank.
    ...
    # Gravity = commercial value. Production commercial ML drives G; rank-only
    # (CFB beta) and heuristic paths use commercial-reweighted BPXVR (college
    # overlay may later replace G with commercial_viability_score).
    if not fallback_used and not rank_only:
    ...
    return ScoreAthleteResponse(
        ...
        value_score_source=value_score_source,
