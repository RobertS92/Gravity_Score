...
def _components_from_raw(raw: dict[str, Any], sport: str) -> dict[str, float]:
    ...
    proximity = min(100.0, max(20.0, 30.0 + math.log1p(nil_val) * 3.0 + trends * 0.25))
...
def _quality_from_components(components: dict[str, float], raw: dict[str, Any]) -> float:
...
def _dollar_bands(gravity: float, nil_val: float, sport: str) -> tuple[float, float, float]:
    base = max(25_000.0, min(75_000_000.0, gravity * 22_000.0 + math.log1p(nil_val) * 8000))
    if nil_val > 0:
        base = max(base, nil_val * 0.35)
...
def score_athlete(req: ScoreAthleteRequest) -> ScoreAthleteResponse:
    ...
    if quality is None:
        quality = _quality_from_components(components, raw)

    nil_val = _f(raw, "nil_valuation")
    if dollar_p50 is None:
        p10, dollar_p50, p90 = _dollar_bands(gravity, nil_val, sport)
    else:
        p10, _, p90 = _dollar_bands(gravity, nil_val, sport)
        p10 = dollar_p50 * 0.65
        p90 = dollar_p50 * 1.55
    ...
    return ScoreAthleteResponse(
        ...
        dollar_confidence={"source": "ml" if not fallback_used else "heuristic", "quality": "moderate"},
...
def score_team(req: ScoreTeamRequest) -> ScoreTeamResponse:
    ...
    qb = loader.resolve(quality_key)
    if qb:
        pred = _predict_from_bundle(qb, raw)
        if pred:
            quality = min(100.0, max(0.0, pred["primary"]))
