#!/usr/bin/env python3
"""
Quick CFB Test - 3 Players
===========================

Fast validation for CFB scraper with 3 notable players.

Usage:
    python quick_test_cfb.py
...
# Test players - 3 notable CFB stars
TEST_PLAYERS = [
...
def validate_player(player_data: CFBPlayerData, player_name: str) -> Dict:
...
def main():
    ...
    # Collect data
    with tqdm(total=3, desc="🏈 Collecting CFB players", unit="player") as pbar:
    ...
    output_dir = f"test_results/CFB_Quick/{timestamp}"
    os.makedirs(output_dir, exist_ok=True)
    
    # CSV
    csv_file = os.path.join(output_dir, f"cfb_quick_{timestamp}.csv")
    df = pd.DataFrame([asdict(p) for p in results])
    df.to_csv(csv_file, index=False)
    
    # Report
    report_file = os.path.join(output_dir, f"cfb_quick_report_{timestamp}.txt")
    with open(report_file, 'w') as f:
        f.write("CFB QUICK TEST REPORT\n")
        f.write("="*70 + "\n\n")
