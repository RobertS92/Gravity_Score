"""
NCAA Basketball (NCAAB) Player Data Scraper
Supports: Men's and Women's college basketball
"""
...
# ============================================================================
# NCAAB TEAMS AND CONFERENCES
# ============================================================================

def get_power_conference_teams() -> Dict[str, str]:
    """Get Power conference teams (ACC, Big Ten, Big 12, SEC, Big East)"""
    return {
        # ACC
...
def get_ncaab_conferences() -> Dict[str, List[str]]:
    """Get conference to team mapping"""
    return {
        "ACC": ["BC", "CLEM", "DUKE", "FSU", "GT", "LOU", "MIA", "UNC", "NCST", 
                "ND", "PITT", "CUSE", "UVA", "VT", "WAKE", "CAL", "STAN", "SMU"],
        ...
        "Big 12": ["ARIZ", "ASU", "BAY", "BYU", "CIN", "COLO", "HOU", "ISU", "KU",
                   "KSU", "OKST", "TCU", "TTU", "UCF", "WVU", "UTAH"],
        "Big East": ["BUT", "CONN", "CREIG", "DEP", "GTWN", "MARQ", "PROV", "SHU",
                     "STJ", "NOVA", "XAV"],
    }


# ============================================================================
# NCAAB PLAYER COLLECTOR
# ============================================================================

class NCAABPlayerCollector:
    """Main class to orchestrate NCAA Basketball data collection"""
    
    def __init__(self, firecrawl_api_key: str, gender: str = "mens"):
    ...
    def collect_player_data(self, player_name: str, team: str, position: str):
        """Collect comprehensive NCAAB player data"""
        ...
        try:
            ...
            logger.info("📋 Collecting identity data from ESPN NCAAB API...")
    ...
    def _get_conference_for_team(self, team: str) -> str:
        ...
        conferences = get_ncaab_conferences()
    ...
    def _collect_identity(self, player_name: str, team: str, position: str):
        """Collect identity data from ESPN NCAAB API."""
        identity = NCAABIdentityData()
        
        # Use ESPN NCAAB API directly
        if self.direct_api:
            logger.info(f"🏀 ESPN NCAAB API: Fetching {player_name}...")
            espn_data = self.direct_api.get_complete_ncaab_player_data(player_name, team, self.gender)
            ...
            if espn_data and espn_data.get("identity"):
                ...
                # Log detailed info
                logger.info(f"✅ ESPN NCAAB API: {player_name} - {player_info.get('team', '?')}, {player_info.get('position', '?')}")
                logger.info(f"   Hometown: {identity.hometown}, Class: {identity.class_year}")
                
                # Log awards summary
                all_american = espn_data.get("all_american", 0)
                if all_american:
                    logger.info(f"   🏆 Awards: All-American={all_american}")
                ...
                if identity.college and identity.class_year:
                    try:
                        ...
                        recruiting_data = recruiting_collector.collect_recruiting_data(
                            ...
                            sport='nba'  # NCAAB players will go to NBA
                        )
        ...
        logger.warning(f"ESPN NCAAB API failed for {player_name}")
        return identity
    ...
    def _collect_brand(self, player_name: str):
        ...
        try:
            ...
            news_data = api.get_google_news_rss(f"{player_name} college basketball")
            if news_data:
        ...
        if hasattr(self, 'free_collector') and self.free_collector:
            try:
                trends_data = self.free_collector.trends.get_trends_score(player_name, "college basketball")
    ...
    def _collect_proof(self, player_name: str, position: str):
        """Collect NCAAB stats and proof data from cached ESPN data."""
        ...
        if self._espn_player_data and self._espn_player_data.get("stats"):
            logger.info(f"📊 Using cached ESPN NCAAB stats for {player_name}...")
            ...
            # Log stats summary
            logger.info(f"✅ ESPN NCAAB Stats: {proof.career_games} games, {proof.career_ppg} PPG, {proof.career_rpg} RPG")
    ...
    def _export_combined_files(self, all_players: List, mode: str = "all"):
        ...
        if not hasattr(self, 'output_folder'):
            ...
            gender_folder = "NCAAB_Mens" if self.gender == "mens" else "NCAAB_Womens"
            self.output_folder = os.path.join(self.scrapes_base_dir, gender_folder, timestamp)
        
        os.makedirs(self.output_folder, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Export combined JSON
        gender_suffix = "mens" if self.gender == "mens" else "womens"
        json_filename = f"ncaab_{gender_suffix}_{mode}_players_{timestamp}.json"
        ...
        # Export combined CSV
        csv_filename = f"ncaab_{gender_suffix}_{mode}_players_{timestamp}.csv"
...
def print_usage():
    ...
    print("""
NCAA Basketball Player Data Collector - Usage
...
Command Line Usage:
  python gravity/ncaab_scraper.py [mode] [args...] [--womens]
  
  Examples:
    # Single player (Men's)
    python gravity/ncaab_scraper.py player "Cooper Flagg" "Duke" "F"
    
    # Single player (Women's)
    python gravity/ncaab_scraper.py player "Paige Bueckers" "UConn" "G" --womens
    
    # Team roster
    python gravity/ncaab_scraper.py team "DUKE"
    
    # Conference
    python gravity/ncaab_scraper.py conference "ACC"
    
    # Power 5 conference teams only (ACC, Big Ten, Big 12, SEC, Big East) - RECOMMENDED
    python gravity/ncaab_scraper.py power5
    python gravity/ncaab_scraper.py power5 --womens
    
    # Test mode (one player per Power 5 team)
    python gravity/ncaab_scraper.py test
    python gravity/ncaab_scraper.py test --womens
    
    # All Power conference teams
    python gravity/ncaab_scraper.py all
""")
...
def main():
    ...
    # Check for womens flag
    gender = "mens"
    if "--womens" in args or "-w" in args:
        gender = "womens"
        args = [a for a in args if a not in ["--womens", "-w"]]
    
    is_womens = (gender == "womens")
    
    collector = NCAABPlayerCollector(api_key, gender)
    
    # Create output folder
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    gender_folder = "NCAAB_Mens" if gender == "mens" else "NCAAB_Womens"
    collector.output_folder = os.path.join(collector.scrapes_base_dir, gender_folder, timestamp)
    ...
    if mode == "player":
        # Single player mode
        if len(args) < 4:
            logger.error("Usage: ncaab_scraper.py player <name> <team> <position>")
            sys.exit(1)
        ...
        # Team roster mode
        if len(args) < 2:
            logger.error("Usage: ncaab_scraper.py team <team_abbrev>")
            sys.exit(1)
        ...
        if team_abbrev not in teams:
            # Try to find team by ID from ESPN
            if collector.direct_api:
                all_teams = collector.direct_api.get_ncaab_teams(gender)
        ...
        if collector.direct_api and team_id:
            roster = collector.direct_api.get_ncaab_team_roster(team_id, gender)
        ...
        # Conference mode
        if len(args) < 2:
            logger.error("Usage: ncaab_scraper.py conference <conference_name>")
            sys.exit(1)
        ...
        for abbrev in tqdm(team_abbrevs[:3], desc=f"Collecting {conf_name}"):  # Limit for testing
            team_name = teams.get(abbrev, abbrev)
            logger.info(f"Would collect roster for {team_name}")
    
    elif mode == "power5":
        # Power 5 conference teams only (ACC, Big Ten, Big 12, SEC, Big East)
        teams = get_power_conference_teams()
        gender_label = "Women's" if is_womens else "Men's"
        logger.info(f"🏀 Collecting {gender_label} data for all {len(teams)} Power 5 teams...")
        ...
        for team_abbrev, team_name in tqdm(teams.items(), desc="Power 5 Teams"):
            try:
                if collector.direct_api:
                    ncaab_teams = collector.direct_api.get_ncaab_teams(gender=gender)
                    team_info = ncaab_teams.get(team_abbrev, {})
                    team_id = team_info.get("id")
                    
                    if team_id:
                        roster = collector.direct_api.get_ncaab_team_roster(team_id, gender=gender)
        ...
        teams = get_power_conference_teams()
        gender_label = "Women's" if is_womens else "Men's"
        logger.info(f"🧪 Test mode: Collecting ONE {gender_label} player per Power 5 team...")
        ...
        teams = get_power_conference_teams()
        gender_label = "Women's" if is_womens else "Men's"
        logger.info(f"🏀 Collecting {gender_label} data for all {len(teams)} Power conference teams...")
        logger.info("💡 Tip: Use 'power5' mode for faster results")
