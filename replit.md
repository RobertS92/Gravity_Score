...
- **Database-First Architecture**: 100% PostgreSQL-driven with NO CSV files or hardcoded content (October 2025).
...
- **Technical Implementations**:
    ...
    - **Market Dashboard**: Flask backend with live PostgreSQL queries displaying Financial KPIs, Top Performers, Market Activity, and Analytics across all pages with ECOS↔NFL toggle.
...
- **Database Management**: 
    - PostgreSQL `comprehensive_nfl_players` table (779 players: 183 ECOS Bills/Dolphins, 596 other NFL)
    - `DatabaseUpdater` handles existing player updates, merging new data with existing records, and preserving historical data
    - `DataProcessor` class exclusively queries PostgreSQL (psycopg2) with no CSV fallbacks - returns empty DataFrames on error
    - All dashboard endpoints (`/api/financial-overview`, `/api/top-performers`, `/api/market-activity`, etc.) query live database data
